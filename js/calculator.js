...
// Map slider value (1..15) to a semantic tier so users can see the meaning
...
function updateSliderUI() {
  ...
  if (tvVal)  tvVal.textContent  = slider.value;
...
// ── Vehicle pills (R43) ──────────────────────────────────────
// The floating panel's segmented vehicle picker. Each .fp-pill button has
// a data-cat attribute. Click → window.setVehicleCat(cat). A vestigial
// .veh-toggle group from older UI lives off-screen; setVehicleCat keeps
// every group in sync via the `vehiclechange` event regardless.
function _syncFpPills() {
  const current = (typeof window.getVehicleCat === 'function') ? window.getVehicleCat() : 'cat2';
  const group = document.getElementById('fp-pills');
  if (!group) return;
  const pills = group.querySelectorAll('.fp-pill');
  let activeIdx = 1; // default to cat2 if nothing matches
  pills.forEach((pill, i) => {
    const isActive = pill.dataset.cat === current;
    pill.setAttribute('aria-checked', isActive ? 'true' : 'false');
    pill.classList.toggle('is-active', isActive);
    if (isActive) activeIdx = i;
  });
  group.dataset.active = String(activeIdx);
}
function _initFpPills() {
  const group = document.getElementById('fp-pills');
  if (!group) return;
  group.querySelectorAll('.fp-pill').forEach(pill => {
    pill.addEventListener('click', () => {
      if (pill.dataset.cat && typeof window.setVehicleCat === 'function') {
        window.setVehicleCat(pill.dataset.cat);
      }
    });
  });
  _syncFpPills();
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', _initFpPills);
} else {
  _initFpPills();
}
window.addEventListener('vehiclechange', _syncFpPills);
